...
\begin{frame}
	...
	\begin{gather*}
		\begin{split}
			\emph{d(V_{m,e}, V_{h,t})} \leftarrow &\; p(h,m), p(h,t), p(t,e), p(t,i), f(h), f(m), f(e),\\
			&\; p(h, V_{m,t}), \emph{p(V_{h,t}, V_{m,e})}, p(V_{h,t}, V_{m,i}), p(V_{h,t}, V_{t,e}),\\
			&\; p(V_{h,t}, V_{t,i}) ,p(t, V_{e,i}), f(V_h, m), f(V_{h,e}), \emph{f(V_{m,e})}
		\end{split}
	\end{gather*}
\end{frame}
...
\begin{frame}
	\frametitle{Bottom-up -- Inverse Resolution}
		...
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}[scale=0.6]
					\node[text width=3cm] (A) at (0, 0) {\tiny{$daughter(mary, ann) \leftarrow female(mary),
					parent(ann, mary)$}};
					\node[text width=3cm] (B) at (8, 0) {\tiny{$daughter(X, Y)\leftarrow female(X), parent(Y,X)$}};
